...
import (
	...
	"github.com/anbarasantr/newrelic-cli/newrelic"
	"github.com/anbarasantr/newrelic-cli/tracker"
	"github.com/anbarasantr/newrelic-cli/utils"
)

var alertsconditionsCmd = &cobra.Command{
	Use:     "alertsconditions <condition-id>",
	Short:   "Upsert alerts_conditions from a file.",
	Aliases: []string{"ac", "alertcondition", "alertscondition"},
	Example: "nr upsert alertsconditions <policy-id> -f <example.yaml>",
	Args: func(cmd *cobra.Command, args []string) error {
		if len(args) != 1 {
			var err = fmt.Errorf("Please give policy id, (nr get alertsconditions <policy-id> [flags])")
			fmt.Println(err)
			os.Exit(1)
			return err
		}
		...
		_, resp, err := client.AlertsConditions.Upsert(context.Background(), conditionsOptions, cat, ac, id)
		if err != nil {
		...
		fmt.Printf("Response status code: %d. Update alert conditions", statusCode)
		if statusCode >= 400 {
			os.Exit(1)
			return
		}
...
func init() {
	UpsertCmd.AddCommand(alertsconditionsCmd)
